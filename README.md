...
Each build also updates the stable tag used by the Bicep default: `odoo:demo-latest`, `postgres:16`, `odoo-mcp:latest`, or `caddy-odoo:latest`. The custom Odoo image deliberately uses a demo-specific tag so it cannot be confused with a mirrored stock Odoo 18 image. Immutable revision tags remain the values used by the normal `azd up` workflow.
